...
export default function Payment() {
  ...
  let { cartId, products } = useContext(cartContext);

  // Create a map of product ID to product details for enrichment
  const productMap = {};
  ...
  // Enrich order items with product details from cart
  const enrichOrderData = (orderData) => {
    if (orderData && orderData.cartItems && Array.isArray(orderData.cartItems)) {
      orderData.cartItems = orderData.cartItems.map(item => {
        const productId = item.product?._id || item.product?.id || item.product;
        // If product is just an ID and we have it in our map, attach the full product
        if (typeof item.product === 'string' && productMap[productId]) {
          item.product = productMap[productId];
        }
        return item;
      });
    }
  ...
  function cashOrder(values) {
    ...
    axios.post(API_ENDPOINTS.ORDERS(cartId), shippingAddress, {
      headers: {
        token: localStorage.getItem(APP_CONFIG.TOKEN_KEY)
      }
    }).then((res) => {
      if (res.data.status === 'success') {
        const enrichedOrder = enrichOrderData(res.data.data);
        localStorage.setItem(APP_CONFIG.CART_STORAGE_KEY, JSON.stringify(enrichedOrder));
      }
    }).catch(() => {
  ...
  function onlineOrder(values) {
    ...
    axios.post(API_ENDPOINTS.CHECKOUT_SESSION(cartId), shippingAddress, {
      headers: {
        token: localStorage.getItem(APP_CONFIG.TOKEN_KEY)
      }
    }).then((res) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: () => {
      // Form submission is handled by button handlers
    },
